...
Vue 中的 slot（插槽）用于 **组件内容的定制与扩展**，设计思想来源于 Web Components。

什么是插槽
- 插槽本质是一个 **占位符**
- 父组件向子组件传入 HTML 片段
- 子组件在对应的 slot 位置进行渲染（内容分发）
...
`Vue 3 中的 slot 方案`在 **使用方式、实现机制和性能** 上都做了统一和优化。

Vue 3 对 slot 的整体改进：

- slot **统一为函数**
- 不再区分「普通插槽 / 作用域插槽」的实现模型
- 插槽本质：`slots[name] = (props) => VNode[]`
...
子组件使用：
- 通过 `slots.xxx(props)` 显式调用

Vue 3 slot 的实现核心：

- 父组件：
  - 编译阶段将插槽内容编译为函数
  - 存放在 `vnode.children.slots` 中

- 子组件：
  - 渲染时通过 `renderSlot(slots, name, props)`
  - 执行插槽函数
  - 获取返回的虚拟节点数组
...
## 双向绑定（v-model）的理解与实现原理总结

`什么是双向绑定`
- **数据变化 → 视图更新**
- **视图变化 → 数据更新**
- Vue 中通过 **`v-model` 指令**实现
- 主要用于**可被用户修改的表单元素**

`v-model 的本质`

**v-model ≠ 单纯的 `value + input` 语法糖**

- 会根据 **元素类型** 生成不同的绑定逻辑
- 内部包含 **指令逻辑 + 事件处理 + 边界场景处理**
...
- `input[type=text]`
  - `value`
  - `input` 事件
  - **处理中文输入（compositionstart / compositionend）**
- `checkbox`
  - `checked`
  - `change` 事件
- `select / radio`
...
=
<MyComp :value="value" @input="value = $event" />
```
